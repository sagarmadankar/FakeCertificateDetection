...
//     try {
//         const fileContent = await fs.readFile('result.json', 'utf-8');
...
//         if (verdict) {
//             const verdictData = JSON.stringify({ verdict });
//             await fs.writeFile('verdict.json', verdictData, 'utf-8');
//         }
...
async function areAllKeyStringsPresent(keyString, matchingThreshold = 0.85) {
    try {
        ...
        if (verdict) {
            const verdictData = JSON.stringify({ verdict: true });
            await fs.writeFile('verdict.json', verdictData, 'utf-8');
        } else {
            const verdictData = JSON.stringify({ verdict: false });
            await fs.writeFile('verdict.json', verdictData, 'utf-8');
        }
